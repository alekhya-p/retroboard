<template>
  <AppPage variant="narrow">
    <div class="edit-board-header">
      <h1>Edit Retro Board</h1>
    </div>
    <form class="edit-board-form surface" @submit.prevent="handleSubmit" novalidate>
      ...
      <div class="form-actions">
        <button type="button" class="btn btn--subtle" @click="router.push(`/boards/${boardId}`)">Cancel</button>
...
const loadBoard = async () => {
  try {
    ...
    form.value = {
      ...
      configurations: {
        show_all_messages: board.configurations.show_all_messages,
        enable_likes: board.configurations.enable_likes
      }
    };
  } catch (err) {
    error.value = 'Failed to load board';
    formMsg.value = 'Failed to load board.';
...
const handleSubmit = async () => {
  ...
  try {
    loading.value = true;
    await boardStore.updateBoard(boardId.value, {
      ...form.value,
      facilitator_id: authStore.user.id,
      block_id: authStore.user.user_type === 'google' ? authStore.user.id : form.value.block_id
    });
    formMsg.value = 'Board updated successfully!';
    formMsgType.value = 'success-msg';
    setTimeout(() => router.push(`/boards/${boardId.value}`), 1000);
  } catch (error) {
    formMsg.value = 'Failed to update board. Please try again.';
    formMsgType.value = 'error-msg';
